...
export default function CourseCard({ course, className, onJoinCourse }: CourseCardProps) {
			...
			<Card className={cn("h-full transition-colors hover:bg-muted/50", className)}>
				<CardHeader className="p-0">
					<div className="relative h-48 w-full">
						<Image
							src={course.imageUrl || "https://picsum.photos/seed/course/400/200"}
							alt={course.title}
							fill
				...
				<CardContent className="p-4">
					<div className="space-y-2">
						...
						<p className="text-sm text-muted-foreground line-clamp-2">
							{course.description || `${course.department} - ${course.semester}`}
						</p>
